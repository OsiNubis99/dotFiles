...
myKeys =
  ...
  , ( "M-y"
    , spawn
      "~/.config/rofi/themes/randomiser.sh; rofi -modi 'clipboard:greenclip print' -show clipboard -run-command '{cmd}'"
    )
  ...
  ,
    ...
    ( "M-s"
    , SM.submap $ searchEngineMap $ S.promptSearchBrowser myPromptConfig
                                                          myBrowser
    )
  ,
    -- NamedScratchpads
...
searchEngineMap method = M.fromList
  ...
  , ( (0, xK_s)
    , method
      $ S.searchEngine "stackoverflow" "https://stackoverflow.com/search?q="
    )
